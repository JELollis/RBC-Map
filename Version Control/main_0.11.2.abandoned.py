...
"""
=================================
RBC City Map Application (v0.11.2)
=================================
...
# -----------------------
# Global UI State and Data
# -----------------------

# Layout / Coordinate System Globals
columns = {}
rows = {}
...
def insert_initial_data(conn: sqlite3.Connection) -> None:
    ...
    initial_data = [
        ...
        ("INSERT OR IGNORE INTO userbuildings (ID, Name, Column, Row) VALUES (?, ?, ?, ?)", [
            ...
            (100, 'Soledad''s Cavern of Shadows', 'Zinc', 'NCL'),
...
def load_data(db_path: str = DB_PATH) -> tuple[
    ...
    dict[Any, tuple[int, int]], dict[Any, QColor], dict[Any, tuple[int, int]], dict[Any, tuple[int, int]], dict[
        Any, tuple[int, int]], dict[str, Any], dict[str, str], int, str, dict[str, str]]:
    """
    ...
    color mappings, shops, guilds, places of interest, settings, theme settings, keybind configuration,
    current CSS profile, and custom CSS rules for that profile.
    ...
    Returns:
        tuple: Contains:
            ...
            - settings (dict[str, Any]): All settings from the settings table.
            - theme_settings (dict[str, str]): Filtered theme settings from settings.
            - keybind_config (int): Keybind configuration value (default 1).
            - current_css_profile (str): Current CSS profile name (default "Default").
            - css_rules (dict[str, str]): Custom CSS rules for the current profile.
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            theme_settings = {k[6:]: v for k, v in settings.items() if k.startswith("theme_")}
            keybind_config = int(settings.get('keybind_config', 1))
            current_css_profile = settings.get('css_profile', "Default")

            cursor.execute("SELECT element, value FROM custom_css WHERE profile_name = ?", (current_css_profile,))
            css_rules = {element: value for element, value in cursor.fetchall()}
            ...
            return (columns, rows, banks_coordinates, taverns_coordinates, transits_coordinates,
                    ...
                    places_of_interest_coordinates, settings, theme_settings, keybind_config,
                    current_css_profile, css_rules)
...
try:
    ...
    banks_coordinates = {}
    color_mappings = {'default': QColor('#000000')}
    settings = {'keybind_config': 1, 'css_profile': 'Default'}
    theme_settings = {}
    keybind_config = 1
    current_css_profile = "Default"
    css_rules = {}
...
class RBCCommunityMap(QMainWindow):
    ...
    def _finalize_setup(self) -> None:
        ...
        self.setup_keybindings()  # Uses keybind_config from _init_data
...
# -----------------------
    ...
    def setup_keybindings(self) -> None:
        ...
        self.movement_keys = movement_configs.get(keybind_config, movement_configs[1])
    ...
    def update_keybind_menu(self) -> None:
        ...
        self.keybind_wasd_action.setChecked(keybind_config == 1)
        self.keybind_arrow_action.setChecked(keybind_config == 2)
        self.keybind_off_action.setChecked(keybind_config == 0)
        logging.debug(f"Updated keybind menu: WASD={keybind_config == 1}, Arrows={keybind_config == 2}, Off={keybind_config == 0}")
...
# -----------------------
    ...
    def load_theme_settings(self) -> None:
        ...
        Updates color_mappings using settings from the database,
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                ...
                for key_with_prefix, value in settings.items():
                    ...
                    color_mappings[key] = QColor(value)
    ...
    def save_theme_settings(self) -> bool:
        """
        Save each theme setting to the SQLite settings table with 'theme_' prefix.
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                ...
                cursor.executemany(
                    ...
                    ''',
                    [(f"theme_{key}", QColor(color).name()) for key, color in color_mappings.items()]
                )
                conn.commit()
    ...
    def apply_theme(self) -> None:
        ...
        try:
            bg_color = QColor(color_mappings.get("background", "#d4d4d4")).name()
            text_color = QColor(color_mappings.get("text_color", "#000000")).name()
            btn_color = QColor(color_mappings.get("button_color", "#b1b1b1")).name()
    ...
    def change_theme(self) -> None:
        """Open theme customization dialog and apply changes."""
        global color_mappings
        dialog = ThemeCustomizationDialog(self)
        if dialog.exec() == QDialog.accepted:
            # Ensure all values are QColor-compatible before applying
            for key, value in color_mappings.items():
                if not isinstance(value, str) or not value.startswith('#'):
                    color_mappings[key] = QColor(value).name()
            self.apply_theme()
            self.update_minimap()
            logging.info("Theme changed and applied")
...
# -----------------------
    ...
    def firstrun_character_creation(self):
        ...
        dialog.setWindowFlags(dialog.windowFlags() | Qt.WindowStaysOnTopHint)  # Add stay-on-top flag
...
# -----------------------
    ...
    def apply_custom_css(self, css):
        """Fetch current profile from settings and apply CSS rules to the webview."""
        try:
            with sqlite3.connect(DB_PATH) as conn:
                cursor = conn.cursor()

                # 1. Get current profile from settings
                cursor.execute("SELECT setting_value FROM settings WHERE setting_name = 'css_profile'")
                result = cursor.fetchone()
                profile = result[0] if result else "Default"

                # 2. Load CSS rules for the profile
                cursor.execute("SELECT element, value FROM custom_css WHERE profile_name = ?", (profile,))
                css_rules = cursor.fetchall()

            if not css_rules:
                logging.warning(f"No CSS rules found for profile '{profile}'")
                return
            ...
            """
            self.website_frame.page().runJavaScript(script)
            logging.debug(f"Applied CSS profile: '{profile}' with {len(css_rules)} rules.")

        except sqlite3.Error as e:
            logging.error(f"Failed to apply custom CSS: {e}")
    ...
    def on_webview_load_finished(self, success):
        if not success:
            ...
            # Close splash screen when webview is fully loaded
            if self.splash:
    ...
    def switch_css_profile(self, profile_name: str) -> None:
        current_css_profile = profile_name
...
# -----------------------
        ...
        for bank_key in banks_coordinates.keys():
            ...
            if col is not None and row is not None:
                ...
                draw_label_box(
                    ...
                    block_size, block_size // 3, color_mappings["bank"], "BANK"
                )
            else:
        ...
        for name, (column_index, row_index) in taverns_coordinates.items():
            if column_index is not None and row_index is not None:
                draw_label_box(
                    ...
                    block_size, block_size // 3, color_mappings["tavern"], name
                )
        ...
        for name, (column_index, row_index) in transits_coordinates.items():
            if column_index is not None and row_index is not None:
                draw_label_box(
                    (column_index - self.column_start) * block_size,
                    (row_index - self.row_start) * block_size,
                    block_size, block_size // 3, color_mappings["transit"], name
                )
        ...
        for name, (column_index, row_index) in user_buildings_coordinates.items():
            if column_index is not None and row_index is not None:
                draw_label_box(
                    (column_index - self.column_start) * block_size,
                    (row_index - self.row_start) * block_size,
                    block_size, block_size // 3, color_mappings["user_building"], name
                )
        ...
        for name, (column_index, row_index) in shops_coordinates.items():
            if column_index is not None and row_index is not None:
                draw_label_box(
                    (column_index - self.column_start) * block_size,
                    (row_index - self.row_start) * block_size,
                    block_size, block_size // 3, color_mappings["shop"], name
                )
        ...
        for name, (column_index, row_index) in guilds_coordinates.items():
            if column_index is not None and row_index is not None:
                draw_label_box(
                    (column_index - self.column_start) * block_size,
                    (row_index - self.row_start) * block_size,
                    block_size, block_size // 3, color_mappings["guild"], name
                )
        ...
        for name, (column_index, row_index) in places_of_interest_coordinates.items():
            if column_index is not None and row_index is not None:
                draw_label_box(
                    (column_index - self.column_start) * block_size,
                    (row_index - self.row_start) * block_size,
                    block_size, block_size // 3, color_mappings["placesofinterest"], name
                )
    ...
    def find_nearest_bank(self, current_x, current_y):
        ...
        for bank_key, (col_name, row_name) in banks_coordinates.items():
        ...
        if self.destination:
            ...
            if nearest_transit_to_character and nearest_transit_to_destination:
                ...
                char_transit_name = next(
                    name for name, coords in transits_coordinates.items() if coords == char_transit_coords)
                dest_transit_name = next(
                    name for name, coords in transits_coordinates.items() if coords == dest_transit_coords)
...
# -----------------------
    ...
    def apply_theme(self) -> None:
        ...
        global color_mappings
        try:
            bg_color = color_mappings.get('background', QColor('white')).name()
            text_color = color_mappings.get('text_color', QColor('black')).name()
            btn_color = color_mappings.get('button_color', QColor('lightgrey')).name()
...
class ThemeCustomizationDialog(QDialog):
    ...
    def setup_ui_tab(self) -> None:
        ...
        for idx, elem in enumerate(ui_elements):
            ...
            color = QColor(color_mappings.get(elem, '#ffffff'))
    ...
    def setup_minimap_tab(self) -> None:
        ...
        minimap_elements = [
            'bank', 'tavern', 'transit', 'user_building', 'shop', 'guild',
            'placesofinterest', 'street', 'alley'
        ]
    ...
    def change_color(self, element_name: str, color_square: QLabel) -> None:
        global color_mappings
        current_color = QColor(color_mappings.get(element_name, '#ffffff'))
        ...
        """
        if color.isValid():
            hex_color = color.name()  # Always store as hex string
            color_mappings[element_name] = hex_color
            pixmap = QPixmap(20, 20)
            pixmap.fill(QColor(hex_color))
            color_square.setPixmap(pixmap)
            logging.debug(f"Changed color for '{element_name}' to {hex_color}")

    def save_and_accept(self) -> None:
        """Save changes to database and apply them before closing."""
        global color_mappings, theme_settings, settings, keybind_config, current_css_profile
        try:
            with sqlite3.connect(DB_PATH) as conn:
                cursor = conn.cursor()

                # Save all color mappings as hex strings
                for type_, color in color_mappings.items():
                    hex_color = QColor(color).name() if isinstance(color, QColor) else color
                    cursor.execute(
                        "INSERT OR REPLACE INTO color_mappings (type, color) VALUES (?, ?)",
                        (type_, hex_color)
                    )

                conn.commit()
                logging.debug(f"Saved {len(color_mappings)} color mappings to database")

        except sqlite3.Error as e:
            logging.error(f"Failed to save color mappings: {e}")
            QMessageBox.critical(self, "Error", f"Failed to save theme: {e}")
            return

        # Reload global values from the database
        try:
            (
                _, _, _, _, _, _, color_mappings,
                _, _, _, settings, theme_settings,
                keybind_config, current_css_profile
            ) = load_data(DB_PATH)
            logging.debug("Theme settings reloaded after save.")
        except Exception as e:
            logging.error(f"Failed to reload theme settings: {e}")

        # Reapply theme + update minimap
        if self.parent:
...
class CSSCustomizationDialog(QDialog):
    ...
    def pick_color(self, css_item: str, preview: QLabel) -> None:
        ...
        if color.isValid():
            ...
            current_style = preview.styleSheet()
            # Remove any previous background-color to avoid stacking
            new_style = re.sub(r'background-color:\s*#[0-9a-fA-F]{6};?', '', current_style)
            new_style += f" background-color: {hex_color};"
            preview.setStyleSheet(new_style.strip())
            self.save_css_item(css_item, f"background-color: {hex_color};")
            logging.debug(f"Set background color for '{css_item}': {hex_color}")
    ...
    def pick_image(self, css_item: str, preview: QLabel) -> None:
        ...
        if file_path:
            ...
            new_style = re.sub(r'background-image:[^;]+;?', '', current_style)
            new_style = re.sub(r'background-size:[^;]+;?', '', new_style)
            new_style += f" background-image: url({safe_path}); background-size: cover;"
            preview.setStyleSheet(new_style.strip())
            self.save_css_item(css_item, f"background-image: url({safe_path}); background-size: cover;")
            logging.debug(f"Set image for '{css_item}': {safe_path}")
    ...
    def add_shadow(self, css_item: str) -> None:
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                cursor = conn.cursor()
                # Fetch existing style if any
                cursor.execute(
                    "SELECT value FROM custom_css WHERE profile_name = ? AND element = ?",
                    (self.current_profile, css_item)
                )
                result = cursor.fetchone()
                existing_style = result[0] if result else ""
                # Remove any previous box-shadow
                new_style = re.sub(r'box-shadow:[^;]+;?', '', existing_style)
                new_style += " box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.5);"
                # Save updated style
                cursor.execute(
                    "INSERT OR REPLACE INTO custom_css (profile_name, element, value) VALUES (?, ?, ?)",
                    (self.current_profile, css_item, new_style.strip())
                )
                conn.commit()
            self.load_existing_customizations()
            logging.debug(f"Added shadow to '{css_item}'")
        except sqlite3.Error as e:
            logging.error(f"Failed to add shadow to '{css_item}': {e}")
            QMessageBox.critical(self, "Error", f"Failed to add shadow: {e}")

    def save_css_item(self, css_item: str, value: str) -> None:
        """Save or merge a CSS customization into the database."""
        if not value.strip():
            return
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                ...
                # Merge or replace style rules
                style_dict = self._parse_style_string(existing_style)
                new_style_dict = self._parse_style_string(value)
                style_dict.update(new_style_dict)

                # Rebuild the style string
                merged_style = "; ".join(f"{k}: {v}" for k, v in style_dict.items()) + ";"

                # Save to DB
                cursor.execute(
                    "INSERT OR REPLACE INTO custom_css (profile_name, element, value) VALUES (?, ?, ?)",
                    (self.current_profile, css_item, merged_style)
                )
                conn.commit()

                logging.debug(f"Saved CSS for '{css_item}': {merged_style}")
        except sqlite3.Error as e:
            logging.error(f"Failed to save CSS for '{css_item}': {e}")
            QMessageBox.critical(self, "Error", f"Failed to save CSS: {e}")

    def _parse_style_string(self, style: str) -> dict:
    ...
    def save_and_apply_changes(self) -> None:
        ...
        # Always ensure current_profile and settings table are synced
        ...
        self.accept()
        logging.info(f"CSS changes saved and applied with profile '{selected_profile}'")
    ...
    def reset_css_item(self, css_item: str, preview: QLabel) -> None:
        """Reset all styles for a specific CSS item in the current profile."""
        try:
            with sqlite3.connect(DB_PATH) as conn:
                cursor = conn.cursor()
                cursor.execute(
...
class SetDestinationDialog(QDialog):
    ...
    def __init__(self, parent: QWidget = None) -> None:
        ...
        columns_dropdown = QComboBox()
        rows_dropdown = QComboBox()
        self.directional_dropdown = QComboBox()
        for dropdown in (columns_dropdown, rows_dropdown, self.directional_dropdown):
            dropdown.setStyleSheet(dropdown_style)
        self.populate_dropdown(columns_dropdown, self.parent.columns.keys() if self.parent else [])
        self.populate_dropdown(rows_dropdown, self.parent.rows.keys() if self.parent else [])
        self.populate_dropdown(self.directional_dropdown, ["On", "East", "South", "South East"])
        custom_layout.addWidget(QLabel("ABC Street:"))
        custom_layout.addWidget(columns_dropdown)
        custom_layout.addWidget(QLabel("123 Street:"))
        custom_layout.addWidget(rows_dropdown)
...
class ShoppingListTool(QDialog):
    ...
    def coins_in_pocket(self) -> int:
        ...
        try:
            self.sqlite_cursor.execute(
                "SELECT pocket FROM coins WHERE character_id = (SELECT id FROM characters WHERE name = ?)",
                (self.character_name,))
    ...
    def coins_in_bank(self) -> int:
        ...
        try:
            self.sqlite_cursor.execute(
                "SELECT bank FROM coins WHERE character_id = (SELECT id FROM characters WHERE name = ?)",
                (self.character_name,))
            result = self.sqlite_cursor.fetchone()
            return result[0] if result else 0
        except sqlite3.Error as e:
...
if __name__ == "__main__":
    ...
    init_methods = [
        '_init_scraper', '_init_window_properties', '_init_web_profile',
        '_init_ui_state', '_init_characters', '_init_ui_components', '_finalize_setup'
    ]
