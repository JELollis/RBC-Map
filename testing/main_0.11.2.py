...
class ShoppingListTool(QDialog):
    ...
    def coins_in_pocket(self) -> int:
        ...
        if not self.sqlite_cursor:
            return 0
        try:
            self.sqlite_cursor.execute("SELECT pocket FROM coins WHERE character_id = (SELECT id FROM characters WHERE name = ?)", (self.character_name,))
    ...
    def coins_in_bank(self) -> int:
        ...
        if not self.sqlite_cursor:
            return 0
        try:
            self.sqlite_cursor.execute("SELECT bank FROM coins WHERE character_id = (SELECT id FROM characters WHERE name = ?)", (self.character_name,))
            result = self.sqlite_cursor.fetchone()
            return result[0] if result else 0
        except sqlite3.Error as e:
