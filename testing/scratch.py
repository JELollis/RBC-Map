...
def setup_logging():
    ...
    """
    log_filename = datetime.now().strftime('./logs/mouse_press_event_%Y-%m-%d.log')
    logging.basicConfig(
...
class RBCCommunityMap(QMainWindow):
    ...
    def extract_coordinates_from_html(self, html):
        """
        Extracts coordinates and context from the HTML content, including city limits and buildings.

        Args:
            html (str): HTML content as a string.

        Returns:
            tuple: (x, y) coordinates for the character's actual position.
        """
        soup = BeautifulSoup(html, 'html.parser')
        table = soup.find('table')  # Locate the main table containing the grid

        if not table:
            logging.error("No table found in the HTML content.")
            return None, None

        cells = table.find_all('td')  # Get all table cells
        if len(cells) < 9:
            logging.warning(f"Expected at least 9 cells, but found {len(cells)}.")
            # Proceed with available cells, though the grid may be incomplete

        grid_data = []
        for index, cell in enumerate(cells):
            x_input = cell.find('input', {'name': 'x'})
            y_input = cell.find('input', {'name': 'y'})

            x_value = int(x_input['value']) if x_input else None
            y_value = int(y_input['value']) if y_input else None
        ...
        # Center cell is always index 5 in a 3x3 grid
        center_cell = grid_data[4] if len(grid_data) >= 5 else None
        if not center_cell or center_cell['x'] is None or center_cell['y'] is None:
            logging.error("Center cell does not contain valid coordinates.")
            return None, None

        character_x, character_y = center_cell['x'], center_cell['y']
        logging.debug(f"Center cell coordinates: x={character_x}, y={character_y}")

        # Validate coordinates by checking surrounding cells
        for cell in grid_data:
            if cell['x'] is not None and cell['y'] is not None:
                logging.debug(f"Grid cell {cell['index']} - x={cell['x']}, y={cell['y']}")
    ...
    def draw_minimap(self):
        ...
        font_size = max(8, block_size // 4)  # Dynamically adjust font size, with a minimum of 8
        ...
        def draw_location(column_index, row_index, color, label_text=None):
            ...
            if label_text:
                # Define QRect for wrapping within cell size, ensuring center alignment
        ...
        for i in range(self.zoom_level):
            for j in range(self.zoom_level):
                ...
                # Initialize column_name and row_name
    ...
    def recenter_minimap(self):
        ...
        # Calculate potential start positions to center the character
        column_start = self.character_x - center_offset
        row_start = self.character_y - center_offset

        # Clamp values to ensure the minimap does not go out of bounds (-1 to 201)
        column_start = max(min(column_start, 201 - self.zoom_level), -1)
        row_start = max(min(row_start, 201 - self.zoom_level), -1)
        ...
        # Update the minimap start positions
